...
public abstract class UITweener : MonoBehaviour
{
    ...
    public bool                     ClearEventsOnFinish { get; set; } = false;
    public bool                     ClearEventsOnStart { get; set; } = false;
	[HideInInspector]
...
//#endif
//			GameCore.utils.update.update_Subscribe(DoUpdate);
//	}
...
//		if (Application.isPlaying)
//#endif
//			GameCore.utils.update.update_Unsubscribe(DoUpdate);
	}

	

	void DoUpdate (float deltaTime) {
        delta   = (!ignoreTimeScale) ? deltaTime : deltaTime;
		time    = Time.time;
		...
		if (!mDelayEnded) {
			...
			if (onStart != null) {
                mTemp       = onStart;
				onStart		= new List<EventDelegate>();
                ...
                if (!ClearEventsOnStart) {
                    for (int i = 0; i < mTemp.Count; ++i) {
                        ...
                        if (ed != null && !ed.oneShot)
                            EventDelegate.Add(onStart, ed, ed.oneShot);
                    }
                }
	...
	void Update () {
        delta   = (!ignoreTimeScale)? Time.deltaTime: Time.unscaledDeltaTime;
		time    = Time.time;
	...
	public void AddOnStart		(EventDelegate del) { EventDelegate.Add(onStart,	del); }
	...
	{
		mStarted = false;
        mFactor = 1f; //(amountPerDelta > 0f) ? 1f : 0f;
		Sample(mFactor, false);
	}
